    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
      ...
      <div class="flex justify-between mb-4">
        <input
          v-model="search"
          type="text"
          placeholder="Rechercher un vol + hotel..."
      ...
      <table class="w-full border-collapse border rounded-lg">
        ...
        <tbody>
          <tr v-for="(flighthotel, index) in filteredFlights" :key="flighthotel.id" class="border-b">
            ...
            <td class="p-3">{{ flighthotel.origin.name }}</td>
            <td class="p-3">{{ flighthotel.destination.name }}</td>
            <td class="p-3">{{ formatDate(flighthotel.departure_date) }}</td>
            <td class="p-3">
              <span v-if="flighthotel.flighthotel_class === 'economy'">Économie</span>
              <span v-else-if="flighthotel.flighthotel_class === 'business'">Affaire</span>
              <span v-else>Première</span>
            </td>
            <td class="p-3">
              <span
                @click="openStatusModal(flighthotel)"
                class="px-3 py-1 rounded-full text-white text-sm cursor-pointer"
                :class="{
                  'bg-yellow-500': flighthotel.status === 'pending',
                  'bg-green-500': flighthotel.status === 'approved',
                  'bg-red-500': flighthotel.status === 'rejected'
                }"
                v-text="(flighthotel.status === 'pending') ? 'En attente' : (flighthotel.status === 'approved') ? 'Validé' : 'Rejeté'"
              >
            ...
            <td class="p-3 flex space-x-2">
              ...
              <button
                @click="confirmDelete(flighthotel)"
...
<script setup>
  ...
  const openStatusModal = (flighthotel) => {
  ...
  const closeModal = () => {
    ...
    selectedFlightHotel.value = null;
  ...
  // Pagination
  const totalPages = computed(() => Math.ceil(props.flighthotels.length / itemsPerPage));
  ...
  const confirmDelete = async (flighthotel) => {
    Swal.fire({
        title: "Êtes-vous sûr ?",
        text: `Voulez-vous vraiment supprimer la réservation de ${flighthotel.customer.name} ?`,
        icon: "warning",
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`${AppDatas.baseUrl}/flight-hotel/${flighthotel.id}`);
                ...
                props.flighthotels.splice(props.flighthotels.indexOf(flighthotel), 1);
            } catch (error) {
                Swal.fire({
                    title: "Erreur",
