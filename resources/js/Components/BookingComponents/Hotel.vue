  ...
  <div>
      ...
      <table class="w-full border-collapse border rounded-lg">
        <thead>
          <tr class="bg-gray-200">
            ...
            <th class="p-3 text-left max-w-24">Client</th>
            <th class="p-3 text-left">Destination</th>
            <th class="p-3 text-left">Dates</th>
            <th class="p-3 text-left">Nombre de chambre</th>
        ...
        <tbody>
          ...
          <tr v-for="(hotel, index) in filteredhotels" :key="hotel.id" class="border-b">
            ...
            <td class="p-3">
              <span
                @click="openStatusModal(hotel)"
                class="px-3 py-1 rounded-full text-white text-sm cursor-pointer"
                :class="{
                  'bg-yellow-500': hotel.status === 'pending',
                  'bg-green-500': hotel.status === 'approved',
                  'bg-red-500': hotel.status === 'rejected'
                }"
                v-text="(hotel.status === 'pending') ? 'En attente' : (hotel.status === 'approved') ? 'Validé' : 'Rejeté'"
              >
    ...
    <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
        <h2 class="text-xl font-bold mb-4">Modifier le statut du hotel :</h2>
        ...
        <form @submit.prevent="updateStatus" v-if="selectedStatus === 'rejected'" class="mt-4">
          ...
          <div class="py-2">
            ...
            <input type="date" :min="minDate" v-model="formStatus.arrivalDate" class="w-full border p-2 rounded-lg" />
          </div>
          <div class="py-2">
...
<script setup>
  ...
  // Pagination
  const totalPages = computed(() => Math.ceil(props.hotels.length / itemsPerPage));
  ...
  const updateStatus = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${AppDatas.baseUrl}/hotels/${selectedhotel.value.id}/status`,
        payload
      );
  ...
  const confirmDelete = async (hotel) => {
    Swal.fire({
        title: "Êtes-vous sûr ?",
        text: `Voulez-vous vraiment supprimer la réservation de ${hotel.customer.name} ?`,
        icon: "warning",
        ...
        if (result.isConfirmed) {
            try {
                ...
                props.hotels.splice(props.hotels.indexOf(hotel), 1);
            } catch (error) {
                Swal.fire({
                    title: "Erreur",
